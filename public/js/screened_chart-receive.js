...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateDropdownText() {
        ...
        let cardTitle = document.querySelector(".card-title-dash1");
        if (cardTitle) {
            cardTitle.textContent = `Monthly Total Received Samples (${selectedProvince})`;
        }
        ...
        document.getElementById("yearPrev").textContent = selectedYear1;
        document.getElementById("yearCurrent").textContent = selectedYear2;
    }
    ...
    function getChartDataForMonth(month) {
        ...
        const prev = window.myChart.data.datasets[0].data[monthIndex] || 0;
        const current = window.myChart.data.datasets[1].data[monthIndex] || 0;

        return { prev, current };
    }

    function updateTable() {
        const { prev: totalPrev, current: totalCurrent } = getChartDataForMonth(selectedMonth);
        const incDec = totalCurrent - totalPrev;
        const percentChange = totalPrev ? ((incDec / totalPrev) * 100).toFixed(2) : 0;
        ...
        const color = incDec >= 0 ? "green" : "red";
        document.getElementById("incDec").style.color = color;
        document.getElementById("percentChange").style.color = color;
    }
    ...
    function updateChart(data) {
        ...
        const responseData = data.monthlyData;
        if (!Array.isArray(responseData) || responseData.length === 0) {
            console.warn("⚠️ No valid received data! Check API response.");
            return;
        }

        const allMonths = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
        const valuesYear1 = Array(12).fill(0);
        const valuesYear2 = Array(12).fill(0);

        responseData.forEach(entry => {
            const year = parseInt(entry.year);
            const monthIndex = parseInt(entry.month) - 1;
            const province = (entry.province || "").trim().toLowerCase();
            const selectedProvinceNormalized = selectedProvince.trim().toLowerCase();

            if (province === selectedProvinceNormalized) {
                if (year === selectedYear1) {
                    valuesYear1[monthIndex] += entry.total_labno || 0;
                }
                if (year === selectedYear2) {
                    valuesYear2[monthIndex] += entry.total_labno || 0;
                }
            }
        });
    ...
    function fetchChartData() {
        const fromDate = `${selectedYear1}-01-01`;
        const toDate = `${selectedYear2}-12-31`;
        const apiUrl = `http://localhost:3001/api/inc-dec/monthly-labno-count?from=${encodeURIComponent(fromDate)}&to=${encodeURIComponent(toDate)}&province=${encodeURIComponent(selectedProvince)}&type=Received`;
    ...
    document.getElementById("yearDropdown1").addEventListener("click", function (event) {
        ...
        const selected = event.target.getAttribute("data-year");
        if (selected) {
            selectedYear1 = parseInt(selected, 10);
            updateDropdownText();
            fetchChartData();
    ...
    document.getElementById("provinceDropdown").addEventListener("click", function (event) {
        event.preventDefault();
        const selected = event.target.getAttribute("data-province");
        if (selected) {
            selectedProvince = selected;
            updateDropdownText();
            fetchChartData();
    ...
    document.getElementById("monthDropdown").addEventListener("click", function (event) {
        event.preventDefault();
        const selected = event.target.getAttribute("data-month");
        if (selected) {
            selectedMonth = selected;
            updateDropdownText();
            updateTable();
