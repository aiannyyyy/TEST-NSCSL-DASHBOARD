...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let selectedYear2 = new Date().getFullYear();     // current year
    let selectedProvince = "Batangas"; // Default province
    // Fixed to screened samples only
    const monthNames = ["January", "February", "March", "April", "May", "June",
                        "July", "August", "September", "October", "November", "December"];
    let selectedMonth = monthNames[new Date().getMonth()]; // ← auto set to current month
    ...
    function updateDropdownText() {
        ...
        // ✅ Update the card title with the selected province - screened samples only
        let cardTitle = document.querySelector(".card-title-dash1");
        if (cardTitle) {
            cardTitle.textContent = `Monthly Total Screened Samples (${selectedProvince})`;
        }
    ...
    function updateChart(data) {
        ...
        let allMonths = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
        let valuesYear1 = Array(12).fill(0);
        let valuesYear2 = Array(12).fill(0);
    
        monthlyData.forEach(entry => {
            let year = entry.year;
            let monthIndex = entry.month - 1; // Convert to 0-based index
            let province = entry.province ? entry.province.trim().toLowerCase() : "";
            let selectedProvinceNormalized = selectedProvince.trim().toLowerCase();
    
            if (province === selectedProvinceNormalized) {
                if (parseInt(year, 10) === selectedYear1) {
                    valuesYear1[monthIndex] = entry.total_labno || 0;
                }
                if (parseInt(year, 10) === selectedYear2) {
    ...
    function fetchChartData() {
        ...
        // ✅ Updated API URL to match your backend route
        let apiUrl = `http://localhost:3001/api/inc-dec/monthly-labno-count?from=${encodeURIComponent(fromDate)}&to=${encodeURIComponent(toDate)}&province=${encodeURIComponent(selectedProvince)}&type=Screened`;
